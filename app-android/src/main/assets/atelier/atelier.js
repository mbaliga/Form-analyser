...
const defs={
 recurve:{title:'A study in balance.',type:'OLYMPIC RECURVE / BRACED STUDY',material:'Bronze · carbon · walnut',center:[.20,0,0],radius:1.02,yaw:.46,pitch:.12,description:'A complete, original recurve assembly. Select a component to move closer, or separate the parts to inspect its construction.'},
 arrow:{title:'Small details. Straight flight.',type:'TARGET ARROW / COMPONENT STUDY',material:'Carbon · steel · polymer',center:[0,0,0],radius:.43,yaw:.12,pitch:.17,description:'A target arrow with a carbon shaft, three individually modelled vanes, a slotted nock and a rounded target point. Select a part for a close view.'},
 target:{title:'The place it all comes together.',type:'RECURVE TARGET / TEN-RING STUDY',material:'Paper · compressed fibre · timber',center:[0,.34,0],radius:1.13,yaw:.36,pitch:.10,description:'A complete ten-ring target and three-support stand. Geometry here is illustrative; scoring remains in Crocodyl’s independently tested scoring engine.'}
};
...
async function load(key){
 ...
 document.querySelectorAll('[data-model]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.model===key)));const d=defs[key];$('model-type').textContent=d.type;$('model-subtitle').textContent=d.material;canvas.setAttribute('aria-label',`Three-dimensional ${key} model. Drag or use arrow keys to rotate; plus and minus to zoom. Select part buttons for details.`);setDetail(null);reset(false);
 try{
   const response=await fetch(`models/${key}.glb`);if(!response.ok)throw Error('Model fetch failed');const buffer=await response.arrayBuffer();if(token!==state.loadToken||disposed)return;
...
function select(id){const p=state.parts.find(p=>p.id===id);if(!p)return;state.selected=state.selected===id?null:id;state.spin=false;updateSpin();document.querySelectorAll('[data-part]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.part===state.selected)));if(!state.selected){reset();return;}setDetail(p);const sizes={recurve:{riser:.33,grip:.12,limbs:.78,string:.84,sight:.18,stabilisers:.59,rest:.10,hardware:.18},arrow:{shaft:.40,point:.08,nock:.055,vanes:.095,wrap:.12},target:{face:.70,butt:.78,stand:.65,pins:.17}};let center=[...p.hotspot];if(state.exploded)center=vadd(center,p.explode);state.target={center,radius:sizes[state.model][id]||defs[state.model].radius,yaw:state.yaw,pitch:state.pitch};request();save();}
...
function render(time){
 ...
 const aspect=w/h;let radius=state.radius*aspect;if(state.model==='arrow')radius=state.radius*1.03;
...
async function metadataOnly(key){if(!Object.keys(defs).includes(key))return;state.model=key;setDetail(null);document.querySelectorAll('[data-model]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.model===key)));try{const r=await fetch(`models/${key}.glb`);if(!r.ok)throw Error('Unavailable');const {json}=parseGLB(await r.arrayBuffer());state.parts=json.nodes.map(n=>n.extras).filter(Boolean);makeParts();}catch{ $('part-body').textContent='This equipment asset is unavailable in this build.';}['reset','explode','rotate'].forEach(id=>$(id).disabled=true);}
