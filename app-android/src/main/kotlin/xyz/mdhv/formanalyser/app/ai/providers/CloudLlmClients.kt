 ...
 * rather than an exception. Only the hosted clouds are served here; ON_DEVICE and OTHER providers
 * return null (routed elsewhere by the caller).
 */
object CloudLlmClients {
    /** Providers this factory can build a cloud client for. */
    fun handles(provider: Provider) =
        when (provider) {
            Provider.ANTHROPIC,
            Provider.OPENAI,
            Provider.GOOGLE,
    ...
    /**
     * Build the client for [provider], or null if this factory does not serve it (on-device /
     * other). [apiKeyLookup] returns the BYOK key for a provider, or null/blank if none is stored.
     */
    fun forProvider(provider: Provider, apiKeyLookup: (Provider) -> String?): LlmClient? =
        when (provider) {
            ...
            Provider.DEEPSEEK -> DeepSeekClient { apiKeyLookup(Provider.DEEPSEEK) }
            Provider.ON_DEVICE,
            Provider.OTHER -> null
        }

    /** Convenience: the client for a model's provider, or null for non-cloud models. */
    fun forModel(model: CoachModel, apiKeyLookup: (Provider) -> String?) =
