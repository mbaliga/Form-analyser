...
fun InjuryEditScreen(
    ...
    Column(
        ...
        Text(
            if (existing == null) "Log injury" else "Injury",
            style = MaterialTheme.typography.headlineMedium,
            color = Hyle.OnBackground,
        )
        HyleSectionHeader("Regions")
        RegionPicker(selected = regions) { id ->
        ...
        HyleSegmented(listOf("ACUTE", "OVERUSE", "UNKNOWN"), mechanism, { it.lowercase() }) {
            mechanism = it
        }
        HyleSectionHeader("Status")
        HyleSegmented(listOf("ACTIVE", "RECOVERING", "RESOLVED"), status, { it.lowercase() }) {
            status = it
        }
        if (status == "RESOLVED")
            Text("Resolving stamps today as the resolved date.", color = Hyle.OnSurfaceDim)
        OutlinedTextField(
            notes,
            { notes = it.take(500) },
            label = { Text("Notes") },
        ...
        if (existing != null) {
            ...
            documents
                .filter { it.injuryId == existing.id }
                .forEach { doc ->
                    HyleListRow(
                        title = doc.title,
                        ...
                        trailing = {
                            OutlinedButton(onClick = { vm.deleteDocument(doc) }) { Text("Delete") }
                        },
                    )
                }
            OutlinedButton(
        ...
        // opened this screen and picked nothing faced a disabled button, no explanation and no way
        // out. Say why it is disabled, and always offer a way to leave.
        if (regions.isEmpty())
            Text(
                "Pick at least one body region to save this injury.",
...
fun PhysioPlanEditScreen(vm: BodyViewModel, planId: String?, onDone: () -> Unit) {
    ...
    var schedule by
        remember(existing?.id) { mutableStateOf(JsonLists.decode(existing?.scheduleJson).toSet()) }
    var exercises by
        remember(existing?.id) {
            mutableStateOf(
    ...
    Column(
        ...
        Text(
            if (existing == null) "New physio plan" else "Physio plan",
            style = MaterialTheme.typography.headlineMedium,
            color = Hyle.OnBackground,
        )
        OutlinedTextField(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            listOf("MO", "TU", "WE", "TH", "FR", "SA", "SU").forEach { d ->
                FilterChip(
                    selected = d in schedule,
                    onClick = { schedule = if (d in schedule) schedule - d else schedule + d },
                    label = { Text(d) },
                )
            }
        }
                ...
                onValueChange = { v ->
                    exercises =
                        exercises.toMutableList().also {
                            it[i] = Triple(v.take(60), sets, repsHold)
                        }
                },
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                Column {
                    ...
                    HyleStepper(
                        ...
                        { v ->
                            exercises =
                                exercises.toMutableList().also { it[i] = Triple(name, v, repsHold) }
                        },
                        1..10,
                    )
                }
                                ...
                                exercises.toMutableList().also {
                                    it[i] =
                                        Triple(
                                            name,
                                            sets,
                                            (if (v == 0) null else v) to repsHold.second,
                                        )
                                }
                        },
                        0..50,
        ...
        OutlinedButton(
            onClick = {
                exercises = exercises + Triple<String, Int, Pair<Int?, Int?>>("", 3, Pair(10, null))
            },
            modifier = Modifier.fillMaxWidth(),
        ) {
            Text("Add exercise")
        ...
        if (regions.isEmpty())
            Text(
                "Pick at least one target region to save this plan.",
                color = Hyle.OnSurfaceDim,
                style = MaterialTheme.typography.labelMedium,
            )
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
