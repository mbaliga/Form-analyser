...
interface AthleteFeatureDao {
    ...
    @Query("SELECT * FROM goal WHERE athleteId = :athleteId AND state != 'ARCHIVED' ORDER BY state ASC, targetAtMs ASC, createdAtMs DESC")
    ...
    @Query("UPDATE goal SET state = :state, updatedAtMs = :atMs WHERE id = :id") suspend fun setGoalState(id: String, state: String, atMs: Long)
    ...
    @Query("SELECT * FROM session_context WHERE athleteId = :athleteId ORDER BY startedAtMs DESC LIMIT :limit")
    ...
    @Query("SELECT * FROM training_plan WHERE athleteId = :athleteId AND state != 'ARCHIVED' ORDER BY startDate DESC")
    ...
    @Query("SELECT * FROM score_candidate WHERE scoreSessionId = :sessionId AND endIndex = :endIndex ORDER BY candidateIndex ASC")
    ...
    @Query("UPDATE observer_score_event SET status = 'RETRACTED' WHERE id = :id") suspend fun retractObserverEvent(id: String)
}
