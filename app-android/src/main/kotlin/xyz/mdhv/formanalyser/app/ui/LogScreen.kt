...
fun LogScreen(vm: WellnessViewModel, cycleEnabled: Boolean, onDone: () -> Unit) {
    ...
    Column(
        ...
        if (picked == null) {
            LogType.entries
                ...
                .forEach { t ->
                    HyleListRow(
                        title = t.label,
                        subtitle =
                            if (t.privateClass) "private — never leaves this device by default"
                            else null,
                        onClick = { picked = t },
                    )
                }
            ...
            when (picked) {
                ...
                LogType.LIFE ->
                    LifeEventForm(vm) { impact ->
                        if (impact < 3) onDone() /* impact 3 stays for the hiatus offer */
                    }
    ...
    if (hiatusOffer != null) {
        AlertDialog(
            onDismissRequest = {
                vm.declineHiatusOffer()
                onDone()
            },
            title = { Text("Pause for a while?") },
            text = {
                Text(
                    "That's a heavy one. Want to pause the streak and quiet the app for a while? Nothing breaks, nothing nags — pick it back up whenever."
                )
            },
            confirmButton = {
                Button(
                    onClick = {
                        vm.acceptHiatusOffer()
                        onDone()
                    }
                ) {
                    Text("Pause")
...
private fun CheckinForm(vm: WellnessViewModel, onDone: () -> Unit) {
    ...
    Button(
        onClick = {
            vm.logStandaloneCheckin(energy, sleep, motivation, emptyList(), note.ifBlank { null })
            onDone()
        },
        modifier = Modifier.fillMaxWidth(),
    ) {
...
private fun RestForm(vm: WellnessViewModel, onDone: () -> Unit) {
    ...
    var planned by rememberSaveable { mutableStateOf(true) }
    var note by rememberSaveable { mutableStateOf("") }
    Row(
        verticalAlignment = Alignment.CenterVertically,
        horizontalArrangement = Arrangement.spacedBy(10.dp),
    ) {
        Switch(checked = yesterday, onCheckedChange = { yesterday = it })
        Text(if (yesterday) "For yesterday" else "For today", color = Hyle.OnSurfaceDim)
    }
    Row(
        verticalAlignment = Alignment.CenterVertically,
        horizontalArrangement = Arrangement.spacedBy(10.dp),
    ) {
        Switch(checked = planned, onCheckedChange = { planned = it })
        Text(if (planned) "Planned rest" else "Unplanned", color = Hyle.OnSurfaceDim)
    }
    OutlinedTextField(
        note,
        { note = it.take(200) },
    ...
    Button(
        onClick = {
            vm.logRestDay(
                if (yesterday) LocalDate.now().minusDays(1) else LocalDate.now(),
                planned,
                note.ifBlank { null },
            )
            onDone()
...
private fun MoodForm(vm: WellnessViewModel, onDone: () -> Unit) {
    ...
    Button(
        onClick = {
            mood?.let {
                vm.logMood(it, tags.toList(), note.ifBlank { null })
                onDone()
            }
        },
        enabled = mood != null,
...
private fun LifeEventForm(vm: WellnessViewModel, onSaved: (Int) -> Unit) {
    val categories =
        listOf("BEREAVEMENT", "ILLNESS", "EXAMS", "WORK", "TRAVEL", "RELATIONSHIP", "OTHER")
    var category by rememberSaveable { mutableStateOf("OTHER") }
    var impact by rememberSaveable { mutableStateOf(1) }
    ...
    Row(
        ...
        Text(if (ongoing) "Ongoing" else "One-day", color = Hyle.OnSurfaceDim)
    }
    Button(
        onClick = {
            vm.logLifeEvent(category, impact, title.ifBlank { category.lowercase() }, ongoing)
            onSaved(impact)
        },
        modifier = Modifier.fillMaxWidth(),
    ) {
...
private fun CycleForm(vm: WellnessViewModel, onDone: () -> Unit) {
    var daysAgo by rememberSaveable { mutableStateOf(0) }
    ...
    HyleSegmented(listOf(1, 2, 3), flow ?: 0, { if (it == 0) "·" else "$it" }) {
        if (it != 0) flow = it
    }
    Button(
        onClick = {
            vm.logCycleStart(LocalDate.now().minusDays(daysAgo.toLong()), flow)
            onDone()
        },
        modifier = Modifier.fillMaxWidth(),
    ) {
...
private fun MedicationForm(vm: WellnessViewModel, onDone: () -> Unit) {
    ...
    var taken by rememberSaveable { mutableStateOf(true) }
    OutlinedTextField(
        name,
        { name = it.take(60) },
        label = { Text("Name") },
    ...
    Row(
        ...
        Text(if (taken) "Taken" else "Skipped", color = Hyle.OnSurfaceDim)
    }
    Button(
        onClick = {
            if (name.isNotBlank()) {
                vm.logMedication(name, dose, taken)
                onDone()
            }
        },
...
private fun EventForm(vm: WellnessViewModel, onDone: () -> Unit) {
    ...
    OutlinedTextField(
        title,
        { title = it.take(80) },
        label = { Text("What happened? (e.g. new string)") },
        modifier = Modifier.fillMaxWidth(),
    )
    Button(
        onClick = {
            if (title.isNotBlank()) {
                vm.logEvent(title, emptyList())
                onDone()
            }
        },
