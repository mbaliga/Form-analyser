 ...
 *
 * `@Singleton`-shared via Hilt rather than one `AppPrefs(app)` per ViewModel — no behaviour change,
 * since `context.dataStore`'s own delegate already deduplicates to one DataStore per file name.
 */
@Singleton
