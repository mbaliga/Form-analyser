...
enum class GoalAggregation { LATEST, MAX, MIN, SUM, AVERAGE }
enum class GoalState { ACTIVE, ACHIEVED, PAUSED, ARCHIVED }
...
object GoalEngine {
    fun evaluate(goal: GoalDefinition, observations: List<MetricObservation>): GoalProgress {
        ...
        val current = when (goal.aggregation) {
        ...
        val achieved = when (goal.direction) {
        ...
        val fraction = if (baseline != null && baseline != goal.targetValue) {
            when (goal.direction) {
                GoalDirection.AT_LEAST -> ((current - baseline) / (goal.targetValue - baseline)).coerceIn(0.0, 1.0)
                GoalDirection.AT_MOST -> ((baseline - current) / (baseline - goal.targetValue)).coerceIn(0.0, 1.0)
            }
...
object TrendEngine {
    ...
    fun summarize(observations: List<MetricObservation>): TrendSummary? {
        ...
        val stability = if (abs(mean) < 1e-9) {
        ...
        val slope = if (denom < 1e-12) 0.0 else {
...
data class SessionContextObservation(
    val atMs: Long,
    val defaults: SessionDefaults,
)

/**
 * "Intelligent" here deliberately means explainable local evidence, not opaque prediction:
 * choose the most recent known value independently for each field, then let an explicit pin win.
 */
object SmartDefaultsEngine {
    fun resolve(
        ...
        fun <T> latest(get: (SessionDefaults) -> T?): T? = recent.firstNotNullOfOrNull { get(it.defaults) }
        val inferred = SessionDefaults(
...
enum class BodySignal { PAIN, SORENESS, INJURY, PHYSIO_TARGET }
...
data class RegionSignal(
    ...
    val contextIntensity: Int get() = maxOf(pain, injurySeverity, if (soreness) 3 else 0)
    val signals: Set<BodySignal> get() = buildSet {
